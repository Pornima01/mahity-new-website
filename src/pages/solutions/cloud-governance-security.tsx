...
const A  = "#ff5c8a";
const A2 = "#ffaa55";
const GRID = "rgba(255,92,138,0.22)";
...
const PIPELINE = [
  { label:"Code Commit",     icon:"code"   as IconName, color:"#94a3b8", status:"pass" },
  { label:"OPA Policy Check",icon:"shield" as IconName, color:A,         status:"scan" },
  { label:"SAST Scan",       icon:"lock"   as IconName, color:A2,        status:"scan" },
  { label:"Compliance Gate", icon:"check"  as IconName, color:"#10b981", status:"pass" },
  { label:"Deploy",          icon:"rocket" as IconName, color:"#10b981", status:"pass" },
];
const PipelineDiagram = () => {
  const ref = useRef<HTMLDivElement>(null);
  const inView = useInView(ref,{once:true});
  const [active,setActive] = useState(-1);
  useEffect(()=>{ if(!inView) return; PIPELINE.forEach((_,i)=>setTimeout(()=>setActive(i),i*500+400)); },[inView]);
  return (
    <div ref={ref} className="space-y-3">
      {PIPELINE.map((step,i)=>(
        <motion.div key={i} initial={{ opacity:0,x:20 }} animate={inView?{opacity:1,x:0}:{}} transition={{ duration:0.35,delay:0.1+i*0.08 }}
          className="relative flex items-center gap-4 rounded-xl border px-4 py-3 transition-all duration-500"
          style={{ borderColor:active>=i?`${step.color}55`:"rgba(255,255,255,0.07)", background:active>=i?`${step.color}0d`:"rgba(255,255,255,0.02)" }}>
          {i<PIPELINE.length-1 && (
            <div className="absolute left-[27px] top-full w-px h-3 z-10" style={{ background:active>=i?step.color:"rgba(255,255,255,0.1)" }}/>
          )}
          <div className="flex-shrink-0"><IconBadge name={step.icon} color={active>=i?step.color:"#ffffff30"} size={36}/></div>
          <span className="text-sm font-bold flex-1 transition-colors" style={{ color:active>=i?"rgba(255,255,255,0.9)":"rgba(255,255,255,0.3)" }}>{step.label}</span>
          <motion.div animate={{ opacity:active>=i?1:0, scale:active>=i?1:0.5 }} transition={{ duration:0.3,delay:0.1 }}>
            {step.status==="pass"
              ? <span className="text-xs font-black px-2 py-0.5 rounded-full" style={{ background:"#10b98120",color:"#10b981",border:"1px solid #10b98140" }}>PASS</span>
              : <span className="text-xs font-black px-2 py-0.5 rounded-full" style={{ background:`${A}20`,color:A,border:`1px solid ${A}40` }}>SCAN</span>
            }
          </motion.div>
        </motion.div>
      ))}
...
const STATS = [
  { value:90,  suffix:"%", label:"Reduction in audit cycle duration",                    color:A,         icon:"chart"  as IconName },
  { value:0,   suffix:"",  label:"Critical misconfigurations reaching production",        color:"#10b981", icon:"shield" as IconName },
  { value:100, suffix:"%", label:"Policy coverage enforced automatically in CI/CD",      color:"#8b5cf6", icon:"lock"   as IconName },
];
const CHALLENGES = [
  { icon:"warning" as IconName, color:"#ef4444", label:"Security as a Gate",      text:"Traditional security gates slow teams down and create adversarial dynamics between security and engineering." },
  { icon:"refresh" as IconName, color:A,         label:"Reactive Remediation",    text:"Finding misconfigurations after deployment means expensive, disruptive fixes under pressure." },
  { icon:"lock"    as IconName, color:"#8b5cf6", label:"Compliance Chaos",        text:"Manual audit cycles spanning months drain engineering resources and still miss critical gaps." },
];
const DELIVERABLES = [
  { icon:"code"   as IconName, color:A,         title:"Policy-as-Code in CI/CD",      desc:"Every change is validated before it touches production — OPA policies embedded directly into your pipelines, not bolted on after." },
  { icon:"shield" as IconName, color:A2,        title:"Shift-Left Security",           desc:"Security decisions made by developers, not a separate gate. We train your teams and automate the guardrails." },
  { icon:"chart"  as IconName, color:"#10b981", title:"Audit-Ready Environments",      desc:"Audit cycles shrink from months to days. Your environments are continuously validated, not scrambled before review." },
  { icon:"users"  as IconName, color:"#8b5cf6", title:"Security Team as Enabler",      desc:"Your security team evolves from police to trusted partner — helping developers move fast safely, not blocking them." },
];
const OUTCOMES = [
  { icon:"chart"  as IconName, color:A,         text:"Audit cycles shrink from months to days." },
  { icon:"shield" as IconName, color:"#10b981", text:"Critical-severity misconfigurations in production become a thing of the past." },
  { icon:"users"  as IconName, color:"#8b5cf6", text:"Your security team evolves from 'police' to 'enabler' — trusted partners who help developers move fast safely." },
];

export default function CloudGovernanceSecurity() {
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              <motion.div initial={{ opacity:0,y:12 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.4 }}
                ...
                <span style={{ color:A }}>Cloud Governance &amp; Security</span>
              </motion.div>
              ...
              <div className="mb-7 text-4xl md:text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight text-white">
                <motion.span initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5,delay:0.12 }}
                  className="inline-block mr-[0.25em]">Security is not</motion.span>
                <br/>
                <motion.span initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5,delay:0.22 }}
                  className="inline-block mr-[0.25em] text-white/35">a checkpoint.</motion.span>
                <br/>
                <motion.span initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5,delay:0.32 }}
                  className="inline-block mr-[0.25em]">It's a</motion.span>
                <motion.span initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5,delay:0.4 }}
                  className="inline-block mr-[0.25em]"><GT>property</GT></motion.span>
                <br/>
                <motion.span initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5,delay:0.48 }}
                  className="inline-block mr-[0.25em]">of the system you</motion.span>
                <motion.span initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5,delay:0.56 }}
                  className="inline-block"><GT>design.</GT></motion.span>
              </div>
              <motion.p initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.6,delay:0.68 }}
                className="text-white/50 text-sm sm:text-base lg:text-lg leading-relaxed max-w-xl mb-8 sm:mb-10">
                DevSecOps means security decisions are made by developers, not a separate gate. We implement{" "}
                <span className="text-white/80 font-semibold">guardrails that prevent misconfigurations</span> before they reach production.
              </motion.p>
              <motion.div initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.6,delay:0.8 }}
                ...
                <motion.a href="/contact-us" whileHover={{ scale:1.04 }} whileTap={{ scale:0.96 }}
                  ...
                  Book a Free Security Audit <SvgIcon name="arrow" size={18} color="currentColor"/>
                </motion.a>
            ...
            <motion.div initial={{ opacity:0,x:40 }} animate={{ opacity:1,x:0 }} transition={{ duration:0.7,delay:0.5 }}
              ...
              <p className="text-xs font-bold uppercase tracking-widest mb-6 text-center" style={{ color:A }}>DevSecOps Pipeline — Shift Left</p>
              <PipelineDiagram/>
              <div className="mt-5 pt-5 border-t border-white/[0.08] grid grid-cols-3 gap-3">
                {[{label:"Policy-as-Code",color:A},{label:"Shift-Left",color:A2},{label:"Zero Trust",color:"#8b5cf6"}].map((p,i)=>(
                  <motion.div key={i} initial={{ opacity:0,y:8 }} animate={{ opacity:1,y:0 }} transition={{ delay:1.2+i*0.1 }}
      ...
      <section className="py-10 sm:py-20 md:py-28 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} className="mb-14">
            ...
            <h2 className="text-2xl sm:text-2xl sm:text-4xl md:text-5xl font-black text-white">Why security becomes <GT>everyone's problem</GT></h2>
          </motion.div>
        ...
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} className="mb-14">
            ...
            <p className="text-white/50 text-lg leading-relaxed max-w-3xl pl-5" style={{ borderLeft:`2px solid ${A}45` }}>
              We take a different approach: policy-as-code embedded directly into your CI/CD pipelines, ensuring every change is validated before it touches production. Traditional security gates slow teams down — we replace them with automated guardrails that developers trust.
            </p>
          </motion.div>
      ...
      <section className="py-14 sm:py-20 md:py-28 px-4 sm:px-6 border-t border-white/[0.08]">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} className="mb-12">
            ...
            <h2 className="text-2xl sm:text-2xl sm:text-4xl md:text-5xl font-black text-white">Security that <GT>ships with you</GT></h2>
          </motion.div>
          ...
          <motion.div initial={{ opacity:0,y:30 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.6 }}
            ...
            <motion.div animate={{ y:[-5,5,-5] }} transition={{ duration:4,repeat:Infinity,ease:"easeInOut" }} className="inline-block mb-8">
              <IconBadge name="shield" color={A} size={72}/>
            </motion.div>
            <h2 className="text-2xl sm:text-2xl sm:text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-white mb-4 sm:mb-5 relative z-10">
              Compliance built in.<br/><GT>Not bolted on after.</GT>
            </h2>
            <p className="text-white/55 text-sm sm:text-base lg:text-lg mb-8 sm:mb-10 max-w-md mx-auto relative z-10 leading-relaxed">
              Stop treating security as a final gate. Let's make it a property of every system you ship.
            </p>
